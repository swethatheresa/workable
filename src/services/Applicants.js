...
export const fetchApplicants = async (jobid) => {
    const querySnapshot = await getDocs(query(ref,orderBy("applied_date","desc"),where("joblistingId", "==", jobid)))
...
export const fetchApplicantsByStatus = async (jobid,status) => {
    const querySnapshot = await getDocs(query(ref,orderBy("applied_date","desc"),where("joblistingId", "==", jobid),where("status", "==", status)))
    const applicants = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
    }));
